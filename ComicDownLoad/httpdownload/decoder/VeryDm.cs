...
namespace comicDownLoad
{
    ...
    {
        string hostName = "http://www.veryimapp.com/";
        ...
        public override SearchResult GetSearchComic(string response)
        {
            ...
            BasicComicInfo basicInfo;
            HtmlNode node = doc.DocumentNode.SelectSingleNode("//div[@class='list']");
            ...
            if (collect != null)
            {
                ...
                {
                    basicInfo = new BasicComicInfo();
                    basicInfo.ComicHref = nodeTemp.SelectSingleNode("./p/a").Attributes["href"].Value;
                    basicInfo.ComicName = nodeTemp.SelectSingleNode("./p/a").InnerText;
                    basicInfo.ComicImgUrl = nodeTemp.SelectSingleNode("./a/img").Attributes["src"].Value;
                    queue.Enqueue(basicInfo);
                }
            }
            
            node = doc.DocumentNode.SelectSingleNode("//ul[@class='pagination']");
            
            if (node != null)
            {
                collect = node.SelectNodes("./li");
                ...
                if (collect != null)
                {
                    foreach (HtmlNode nodeTemp in collect)
                    {
                        if (nodeTemp.Attributes["class"] != null)
                        {
                            pos = count;
                        }

                        if (dict.ContainsKey(nodeTemp.ChildNodes["a"].InnerText) == false)
                        {
                            dict.Add(nodeTemp.ChildNodes["a"].InnerText, hostName + nodeTemp.ChildNodes["a"].Attributes["href"].Value);
                        }
            ...
            Regex regex = new Regex(@"找到(?<num>\d+)条关于");
            string temp = regex.Match(response).Groups["num"].Value;
            result.Count = Convert.ToInt32(temp);
